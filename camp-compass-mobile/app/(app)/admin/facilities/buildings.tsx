...
import { Building2, Plus, X, Edit2, Trash2 } from 'lucide-react-native';
...
export default function ManageBuildings() {
  ...
  const [formData, setFormData] = useState({ name: '', code: '', campusId: '', floors: '1' });
  ...
  const openModal = (item: any = null) => {
    if (item) {
      ...
      setFormData({
        ...
        floors: item.floors?.toString() || '1'
      });
    } else {
      setEditingItem(null);
      setFormData({ name: '', code: '', campusId: campuses[0]?.id || '', floors: '1' });
    }
    setModalVisible(true);
  };

  const handleDelete = (id: string) => {
    Alert.alert('Confirm Delete', 'Are you sure you want to delete this building?', [
  ...
  const handleSubmit = async () => {
    if (!formData.name || !formData.campusId) {
      Alert.alert('Error', 'Name and Campus are required');
      return;
    }
    setSubmitting(true);
    try {
      ...
      if (editingItem) {
        await dataService.updateBuilding(editingItem.id, payload as any);
      } else {
        await dataService.createBuilding(payload as any);
      }
        ...
        renderItem={({ item }) => (
          <View style={styles.card}>
            <View style={styles.cardHeader}>
              <View style={styles.iconBg}><Building2 color="#16A34A" size={20} /></View>
              <View style={{ flex: 1 }}>
                <Text style={styles.cardTitle}>{item.name} {item.code ? `(${item.code})` : ''}</Text>
                <Text style={styles.cardSub}>{item.campus?.name} • {item.floors} floors</Text>
              </View>
            </View>
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            <View style={styles.modalHeader}>
              <Text style={styles.modalTitle}>{editingItem ? 'Edit Building' : 'New Building'}</Text>
              <TouchableOpacity onPress={() => setModalVisible(false)}><X color="#6B7280" size={24}/></TouchableOpacity>
            </View>
            
            <Text style={styles.label}>Name</Text>
            <TextInput style={styles.input} value={formData.name} onChangeText={t => setFormData({...formData, name: t})} placeholder="Engineering Block" />
            
            <View style={styles.row}>
              <View style={{flex: 1, marginRight: 8}}>
                <Text style={styles.label}>Code</Text>
                <TextInput style={styles.input} value={formData.code} onChangeText={t => setFormData({...formData, code: t})} placeholder="ENG" />
              </View>
              <View style={{flex: 1}}>
                <Text style={styles.label}>Floors</Text>
                <TextInput style={styles.input} keyboardType="numeric" value={formData.floors} onChangeText={t => setFormData({...formData, floors: t})} />
              </View>
            </View>
            ...
            <TouchableOpacity 
              ...
              disabled={submitting}
            >
              {submitting ? <ActivityIndicator color="#fff" /> : <Text style={styles.submitText}>Save Building</Text>}
            </TouchableOpacity>
          </View>
        </View>
      </Modal>
...
const styles = StyleSheet.create({
  ...
  iconBg: { width: 40, height: 40, borderRadius: 10, backgroundColor: '#DCFCE7', justifyContent: 'center', alignItems: 'center', marginRight: 12 },
  ...
  fab: { position: 'absolute', bottom: 24, right: 24, width: 56, height: 56, borderRadius: 28, backgroundColor: '#16A34A', justifyContent: 'center', alignItems: 'center', shadowColor: '#16A34A', shadowOffset: {width: 0, height: 4}, shadowOpacity: 0.3, shadowRadius: 8, elevation: 5 },
